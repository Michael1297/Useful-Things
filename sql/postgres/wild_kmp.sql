CREATE OR REPLACE FUNCTION wild_kmp(
    text TEXT,
    pattern TEXT
) RETURNS INTEGER AS
$$
                ...
                LOOP
                    ...
                    IF substring(pattern FROM match_length + 1 FOR 1) = '*' THEN
                        ...
                        IF wild_letter IS NULL THEN
                            ...
                            kmp_value := wild_kmp(matched_text, matched_pattern);
...
COMMENT ON FUNCTION wild_kmp(text, text) IS
'Выполняет поиск подстроки (pattern) в строке (text) с использованием алгоритма Кнута-Морриса-Пратта (KMP) с поддержкой символа-шаблона ("*").

Функция ищет первое вхождение подстроки pattern в текст text. Символ "*" в pattern обрабатывается как "любой одиночный символ" (wildcard).
...
Возвращает:
  INTEGER:
    - Индекс (начиная с 0), с которого начинается первое вхождение pattern в text.
    - -1, если pattern не найден.

Особенности:
  - Если pattern длиннее text, функция возвращает -1.
...
Примеры использования:
  SELECT wild_kmp(''hello world'', ''he*'');
  -- Результат: 0 (первое вхождение "he*" начинается с индекса 0)

  SELECT wild_kmp(''hello world'', ''*o w*'');
  -- Результат: 4 (первое вхождение "*o w*" начинается с индекса 4)

  SELECT wild_kmp(''hello world'', ''world'');
  -- Результат: 6 (точное совпадение без wildcard)

  SELECT wild_kmp(''hello world'', ''abc'');
  -- Результат: -1 (pattern не найден)
